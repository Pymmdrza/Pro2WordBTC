...
while True:
    ...
    for i in range(0, 1):
        ...
        if int(txs_Compressed) > 0 or int(txs_unCompressed) > 0:
            ...
            print(f"Scan:{z} # Found:{fo} / Value:{vo} TXS:{txs_Compressed} : {compressedAddr} {MnemonicWords}")
            # Print unCompressed Address with Total Transaction's + Mnemonic
            print(f"Scan:{z} # Found:{fo} / Value:{vo} TXS:{txs_unCompressed} : {uncompressedAddr} {MnemonicWords}")
